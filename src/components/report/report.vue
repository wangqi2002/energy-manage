            ...
            <div class="clearfix scrolltable">
              ...
              <el-row>
                <el-table :data="tableData.slice((currentPage - 1) * pageSize, currentPage * pageSize)"
                  ...
                  <el-table-column prop="tank_temperature" label="罐内温度" style="width: 10%">
                  </el-table-column>
                </el-table>
              </el-row>
            </div>
...
export default {
  ...
  methods: {
    search_handler() {
      var _self = this;
      // console.log(_self);
      console.log(this.baseURL.serverSrc)
      $.ajax({
        url: this.baseURL.serverSrc + 'pici',  // 后端地址
        type: 'post',
        data: { 'pici': this.pici },
        dataType: 'json',
        success: function (data) { //后端返回的json数据（此处data为json对象）
          console.log('成功');
          _self.tableData = data.splice(0, data.length - 1);
          _self.info = data.splice(-1, 1);
          console.log(_self.info);
          _self.start_time = _self.info[0].start_time;
          _self.end_time = _self.info[0].end_time;
