...
export default {
  ...
  methods: {
    search_handler() {
      ...
      console.log(_self);
      $.ajax({
        url: this.baseURL.serverSrc + 'bar',  // 后端地址
        type: 'post',
        ...
        success: function (data) { //后端返回的json数据（此处data为json对象）
          ...
          _self.bardata = val;
          console.log(_self.bardata);
          // console.log(_self.bardata[0]["unit1_power"]);
        },
        error: function () {
          alert('异常')
        }
    ...
    drawBar() {
      ...
      for (let i = 0; i < this.bardata.length; i++) {
        myDataX.push(this.bardata[i]["time"]);
        myDataY1.push(this.bardata[i]["unit1_power"]);
        myDataY2.push(this.bardata[i]["unit2_power"]);
      }
