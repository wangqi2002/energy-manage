...
<!--    </div>-->
    ...
    <div class="water-marketing-wrap">
      <div class="water-marketing-inner">
        <div id="myChartBar2" style="width:100%;height:100%;"></div>
      </div>
    </div>
  ...
  export default{
    ...
    methods: {
      search_handler() {
        ...
        console.log(_self);
        $.ajax({
          url: 'http://106.13.78.117:9090/unit2_water',  // 后端地址
          type: 'post',
          data: {'bgntime': this.bgntime, 'endtime': this.endtime},
          dataType: 'json',
          success: function (data) { //后端返回的json数据（此处data为json对象）
            console.log('成功');
            _self.unit2_data=data;
      ...
      drawBar(){
        ...
        for (let i=0;i< this.unit2_data.length;i++){
          ...
          myDataY1.push(this.unit2_data[i]["unit2_water"]);
        }
        let myChart = this.$echarts.init(document.getElementById('myChartBar2'));
        // 绘制图表
        let option = {
...
<style>
  .mlnewnum{margin-left: .4rem}
  .searchs{width:16px; height: 16px; background: url(../../../static/images/ss.png) no-repeat;position: absolute; margin-left: 0.12rem;top: .09rem}
  .mlnumber{margin-left:.2rem;}
